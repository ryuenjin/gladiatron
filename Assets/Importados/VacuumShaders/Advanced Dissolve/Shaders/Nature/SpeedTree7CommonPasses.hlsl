...
#ifndef SCENESELECTIONPASS
float4 dissolaveAlpha = AdvancedDissolveGetAlpha(input.uvHueVariation.xy, input.positionWS.xyz, input.normalWS, input.dissolveUV);
DoDissolveClip(dissolaveAlpha); 


float3 dissolveAlbedo = 0;
float3 dissolveEmission = 0;
float dissolveBlend = DoDissolveAlbedoEmission(dissolaveAlpha, dissolveAlbedo, dissolveEmission, input.uvHueVariation.xy, 1);
#endif
...
#ifndef SCENESELECTIONPASS
    diffuseColor.rgb = lerp(diffuseColor.rgb, dissolveAlbedo, dissolveBlend);
    emission = lerp(emission, dissolveEmission, dissolveBlend);
#endif


    half4 color = UniversalFragmentBlinnPhong(inputData, diffuseColor.rgb, half4(0, 0, 0, 0), 0, emission, diffuse.a);
