...
SpeedTreeVertexOutput SpeedTree8Vert(SpeedTreeVertexInput input)
{
    ...
    #ifndef SCENESELECTIONPASS
    ADVANCED_DISSOLVE_INIT_DATA(output.clipPos, input.texcoord.xy, input.texcoord1.xy, output)
    #endif
...
#ifndef SCENESELECTIONPASS
float4 dissolaveAlpha = AdvancedDissolveGetAlpha(input.interpolated.uv, input.interpolated.positionWS.xyz, input.interpolated.normalWS, input.interpolated.dissolveUV);
DoDissolveClip(dissolaveAlpha); 


float3 dissolveAlbedo = 0;
float3 dissolveEmission = 0;
float dissolveBlend = DoDissolveAlbedoEmission(dissolaveAlpha, dissolveAlbedo, dissolveEmission, input.interpolated.uv, 1);
#endif
...
#ifndef SCENESELECTIONPASS
float4 dissolaveAlpha = AdvancedDissolveGetAlpha(input.uv, input.positionWS.xyz, input.normalWS, input.dissolveUV);
DoDissolveClip(dissolaveAlpha); 
#endif
