Shader "VacuumShaders/Advanced Dissolve/Lightweight Render Pipeline/TextMeshPro/Bitmap Custom Atlas" {
...
SubShader{
	...
	Pass {
		...
		v2f vert (appdata_t v)
		{
			...
			OUT.positionWS = mul(unity_ObjectToWorld, float4(v.vertex.xyz, 1)).xyz;
            OUT.normalWS = UnityObjectToWorldNormal(v.normal);

			ADVANCED_DISSOLVE_INIT_DATA(OUT.vertex, v.texcoord0.xy, v.texcoord1.xy, OUT);
		...
		{
			float4 dissolaveAlpha = AdvancedDissolveGetAlpha(IN.texcoord0.xy, IN.positionWS.xyz, IN.normalWS, IN.dissolveUV);
            DoDissolveClip(dissolaveAlpha); 

            float3 dissolveAlbedo = 0;
            float3 dissolveEmission = 0;
            float dissolveBlend = DoDissolveAlbedoEmission(dissolaveAlpha, dissolveAlbedo, dissolveEmission, IN.texcoord0.xy, 1);
	...
	Fallback "TextMeshPro/Bitmap Custom Atlas"
